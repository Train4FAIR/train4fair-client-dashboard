...
nopt.invalidHandler = function(k,v,t) {
    ...
    console.log(k,v,t);
    console.log('settings.tmt_ui_env.host: '+settings.tmt_ui_env.host);
    console.log('settings.tmt_ui_env.port: '+settings.tmt_ui_env.port);
}
...
//var res = request('GET', settings.svr_loc_env.protocol+settings.svr_loc_env.host+':'+settings.svr_loc_env.port+settings.svr_loc_env.app_ctx+settings.svr_loc_env.rest_ctx+'/'+env+'/'+type+'/'+token);
console.log('svr_loc_env: '+settings.svr_loc_env.protocol+settings.svr_loc_env.host+':'+settings.svr_loc_env.port+settings.svr_loc_env.app_ctx+settings.svr_loc_env.rest_ctx+'/'+settings.tmt_ui_env.env+'/'+settings.tmt_ui_env.type+'/'+settings.tmt_ui_env.token);
var host = settings.tmt_ui_env.host;
var port = settings.tmt_ui_env.port;

process.env.sd_method = settings.node_config.sd_method || 'GET';
process.env.sd_protocol = settings.svr_loc_env.protocol || 'http://';
process.env.sd_host = settings.svr_loc_env.host || '127.0.0.1';
process.env.sd_port = settings.svr_loc_env.port || '8881';
process.env.sd_env = settings.node_config.sd_env || 'TEST';
process.env.sd_auth_token = settings.node_config.sd_auth_token || 'admin';
process.env.datacite_url  = settings.node_config.datacite_url || 'https://api.test.datacite.org/dois';
process.env.doi_prefix  = settings.node_config.doi_prefix || '10.20408';
process.env.doi_auth_token = settings.node_config.doi_auth_token || 'Basic REVWLkZJVDpOYWhhbkAxMjM=';
process.env.doi_con_contenttype  = settings.node_config.doi_con_contenttype || 'application/vnd.api+json';
// process.env.esb_method = settings.node_config.esb_method || 'GET';
// process.env.esb_protocol = settings.node_config.node_config.esb_protocol || 'http://';
// process.env.esb_host = settings.node_config.esb_host || 'train.platform.de';
// process.env.esb_port = settings.node_config.esb_port || '8080';
// process.env.esb_env = settings.node_config.esb_env || 'TEST';
// process.env.esb_auth_token = settings.node_config.esb_auth_token || 'admin';
...
settings.uiHost = settings.uiHost||settings.tmt_ui_env.host;
...
function getListenPath() {
    ...
    var listenPath = 'http'+(settings.https?'s':'')+'://'+
        //(settings.uiHost == '::'?'menzel.informatik.rwth-aachen.de':(settings.uiHost == '0.0.0.0'?'menzel.informatik.rwth-aachen.de':settings.uiHost))+
        (settings.uiHost == '::'?tmtEnv.host:(settings.tmt_ui_env.host == settings.tmt_ui_env.host?settings.tmt_ui_env.host:settings.tmt_ui_env.host))+
        ':'+settings.tmt_ui_env.port;
    if (settings.httpAdminRoot !== false) {
...
RED.start().then(function() {
    if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
        ...
        //==
        RED.httpAdmin.get('/static/train4fair/javascript/*', function(req, res){
            var options = {
                root: __dirname + '/static/train4fair/javascript',
                dotfiles: 'deny'
            };
        ...
        server.listen(settings.uiPort,settings.uiHost,function() {
            console.log("==[red.js]===>> __dirname: "+__dirname)
            console.log("==[red.js]===>> __dirname /static/: "+__dirname + '/static/')
            if (settings.httpAdminRoot === false) {
                RED.log.info(RED.log._("server.admin-ui-disabled"));
            }
            settings.serverPort = server.address().port;
            process.title = parsedArgs.title || 'Train Modelling Tool - v3.3';//'node-red';
