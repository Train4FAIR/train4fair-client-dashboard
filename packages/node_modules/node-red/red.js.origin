...
if (parsedArgs.help) {
    console.log("Node-RED v"+RedJs.version());
...
RedJs.start().then(function() {
    if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
        server.on('error', function(err) {
            if (err.errno === "EADDRINUSE") {
                RedJs.log.error(RedJs.log._("server.unable-to-listen", {listenpath:getListenPath()}));
                RedJs.log.error(RedJs.log._("server.port-in-use"));
            } else {
                RedJs.log.error(RedJs.log._("server.uncaught-exception"));
                if (err.stack) {
                    RedJs.log.error(err.stack);
                } else {
                    RedJs.log.error(err);
                }
            }
        ...
        server.listen(settings.uiPort,settings.uiHost,function() {
            if (settings.httpAdminRoot === false) {
                RedJs.log.info(RedJs.log._("server.admin-ui-disabled"));
            }
            settings.serverPort = server.address().port;
            process.title = parsedArgs.title || 'node-red';
            RedJs.log.info(RedJs.log._("server.now-running", {listenpath:getListenPath()}));
        });
    } else {
        RedJs.log.info(RedJs.log._("server.headless-mode"));
    }
}).otherwise(function(err) {
    RedJs.log.error(RedJs.log._("server.failed-to-start"));
    if (err.stack) {
        RedJs.log.error(err.stack);
    } else {
        RedJs.log.error(err);
